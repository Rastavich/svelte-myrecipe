...
export default async (req: NowRequest, res: NowResponse) => {
    ...
    if (req.method === "GET") {
        ...
        var importUrl = req.url.match(/(?<=www.)[^]+/g);
        var formatedUrl = importUrl[0].replace(/(%\d)(.)/gm, "/");
        ...
        var importUrlVal = `https://cors-anywhere.herokuapp.com/https://www.` + formatedUrl;
        
        const response = await fetch(importUrlVal, {
            headers: {
            method: "GET",
            Origin: importUrlVal
            },
